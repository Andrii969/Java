...
public class TaskData {
    ...
    // “list vs set” a list supports ordered collections and allows duplicates,
    // whereas a set emphasizes uniqueness without any specific order.

    public static Set<Task> getTasks(String owner) {
        Set<Task> taskList = new HashSet<>();
        String user = ("ann.bob.carol".contains(owner.toLowerCase())) ? owner : null;
        String ownerTasks = switch (owner.toLowerCase()) {
            case "ann" -> annsTasks;
            case "bob" -> bobsTasks;
            case "carol" -> carolsTasks;
            default -> allTasks;
        };
        for (String taskDada : ownerTasks.split("\n")) {
            String[] data = taskDada.split(",");
            Arrays.asList(data).replaceAll(String::trim); // (d) -> d.trim();

            Status status = (data.length <= 3) ? Status.IN_QUEUE :
                    Status.valueOf(data[3].toUpperCase().replaceAll(" ", "_"));
            Priority priority = Priority.valueOf(data[2].toUpperCase());

            taskList.add(new Task(user, data[0], data[1], priority, status));
        }
        return taskList;
    }
}
